...
public class BoundedConcurrencyTest {
  ...
  @Test public void concurrently_concurrencySmallerThanElements() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .collect(withMaxConcurrency(3).concurrently(Integer::parseInt))
                .toMap())
  ...
  @Test public void concurrently_concurrencyLargerThanElements() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .collect(withMaxConcurrency(5).concurrently(Integer::parseInt))
                .toMap())
        .containsExactly("1", 1, "2", 2, "3", 3, "4", 4);
  }
  ...
  @Test public void concurrently_concurrencyEqualToElements() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .collect(withMaxConcurrency(4).concurrently(Integer::parseInt))
                .toMap())
        .containsExactly("1", 1, "2", 2, "3", 3, "4", 4);
  }
  ...
  @Test public void concurrently_concurrencyEqualToOne() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .collect(withMaxConcurrency(1).concurrently(Integer::parseInt))
                .toMap())
        .containsExactly("1", 1, "2", 2, "3", 3, "4", 4);
  }
  ...
  @Test public void concurrently_maxConcurrency() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .collect(withMaxConcurrency(Integer.MAX_VALUE).concurrently(Integer::parseInt))
                .toMap())
        .containsExactly("1", 1, "2", 2, "3", 3, "4", 4);
  }
  ...
  @Test public void concurrently_exceptionPropagated() {
    RuntimeException thrown = assertThrows(
        ...
        () -> Stream.of("1", "2", "3", "four")
            .collect(withMaxConcurrency(2).concurrently(Integer::parseInt))
            .toMap());
  ...
  @Test public void mapConcurrently_concurrencySmallerThanElements() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .gather(withMaxConcurrency(3).mapConcurrently(Integer::parseInt)))
  ...
  @Test public void mapConcurrently_concurrencyLargerThanElements() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .gather(withMaxConcurrency(5).mapConcurrently(Integer::parseInt)))
        .containsExactly(1, 2, 3, 4);
  }
  ...
  @Test public void mapConcurrently_concurrencyEqualToElements() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .gather(withMaxConcurrency(4).mapConcurrently(Integer::parseInt)))
        .containsExactly(1, 2, 3, 4);
  }
  ...
  @Test public void mapConcurrently_concurrencyEqualToOne() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .gather(withMaxConcurrency(1).mapConcurrently(Integer::parseInt)))
        .containsExactly(1, 2, 3, 4);
  }
  ...
  @Test public void mapConcurrently_maxConcurrency() {
    assertThat(
            Stream.of("1", "2", "3", "4")
                .gather(withMaxConcurrency(Integer.MAX_VALUE).mapConcurrently(Integer::parseInt)))
        .containsExactly(1, 2, 3, 4);
  }
  ...
  @Test public void mapConcurrently_exceptionPropagated() {
    RuntimeException thrown = assertThrows(
        RuntimeException.class,
        () -> Stream.of("1", "2", "3", "four")
            .gather(withMaxConcurrency(2).mapConcurrently(Integer::parseInt))
            .toList());
    assertThat(thrown).hasCauseThat().isInstanceOf(NumberFormatException.class);
  }
  ...
  /**
   * Shows that with a heartbeat or monitoring task at the beginning, mapConcurrently() isn't
   * subject to halting.
   */
  ...
  /**
   * Shows that with a heartbeat or monitoring task at the beginning, mapConcurrent() will halt (we
   * use a timeout to avoid halting).
   */
  ...
  @Test public void parallelStream_upstreamFailureDoesNotInterruptDownstream() throws Exception {
    ...
    try (ForkJoinPool pool = new ForkJoinPool(3)) {
      ExecutionException thrown = assertThrows(
          ...
          () -> {
            try {
              pool.submit(() -> asList(10, 3, 1).parallelStream()
                      .map(n -> {
                        if (n == 3) {
  ...
  @Test public void flatMapConcurrently_concurrencySmallerThanElements() {
    assertThat(
            Stream.of(1, 2, 3, 4)
                .gather(
                    withMaxConcurrency(3)
                        .flatMapConcurrently(n -> Collections.nCopies(n, n).stream())))
  ...
  @Test public void flatMapConcurrently_exceptionPropagated() {
    RuntimeException thrown = assertThrows(
        RuntimeException.class,
        () -> Stream.of("1", "2", "3", "four")
            .gather(withMaxConcurrency(2).flatMapConcurrently(s -> Stream.of(Integer.parseInt(s))))
            .toList());
    assertThat(thrown).hasCauseThat().isInstanceOf(NumberFormatException.class);
  }
  ...
  @Test public void race_recoverableFailuresIgnored() {
    ...
    ConcurrentLinkedQueue<Integer> interrupted = new ConcurrentLinkedQueue<>();
    CountDownLatch othersRunning = new CountDownLatch(2);
    List<Callable<String>> tasks = Stream.of(10, 1, 0, 3)
        .<Callable<String>>map(n -> () -> {
          started.add(n);
  ...
  @Test public void
      race_noSuccess_unrecoverableFailuresPropagatedWithRecoverableErrorsSuppressed() {
    ConcurrentLinkedQueue<Integer> started = new ConcurrentLinkedQueue<>();
    ConcurrentLinkedQueue<Integer> interrupted = new ConcurrentLinkedQueue<>();
    CountDownLatch tenStarted = new CountDownLatch(1);
    List<Callable<String>> tasks = Stream.of(1, 0, 2, 10)
        .<Callable<String>>map(n -> () -> {
          started.add(n);
  ...
  @Test public void race_returnsNullSuccessfully() {
    List<Callable<String>> tasks = asList(() -> null);
    assertThat(withMaxConcurrency(1).race(tasks, e -> true)).isNull();
  }

  @Test public void concurrently_threadStartFailure_cancelsPendingAndPropagates() {
    ConcurrentLinkedQueue<Integer> interrupted = new ConcurrentLinkedQueue<>();
    AtomicInteger factoryCallCount = new AtomicInteger();
    ThreadFactory statefulFactory = runnable -> {
  ...
  @Test public void mapConcurrently_platformThreadFactory() {
    ...
    assertThat(
            Stream.of(1, 2, 3, 4)
                .gather(
                    BoundedConcurrency.withMaxConcurrency(2, platformThreads)
                        .mapConcurrently(n -> {
   ...
   * other tasks are still running when the winner finishes" holds regardless of machine speed. The
   * bound is there so that a regression fails the build instead of hanging it.
   */
