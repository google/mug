 ...
 * <p>Determinism notes:
 * <ul>
 * <li>{@link Happenstance#checkpoint} makes the winner finish only after the loser is confirmed
 *     running, so the cancellation is guaranteed to hit a live mapper. No sleep-based racing.
 *     {@code checkpoint()} rather than {@code join()}: the test only needs temporal ordering, and
 *     the extra memory barrier from {@code join()} could hide a visibility bug in the SUT.
 * <li>The input has elements remaining after the short-circuit, so the short-circuit happens inside
 *     {@code integrate} rather than inside {@code finish}. This matters: if the downstream rejects
 *     during {@code finish}'s own flush, {@code finish} returns before re-polling the exception
 *     queue and the bug does not manifest.
...
public class BoundedConcurrencyShortCircuitTest {
  ...
  @Test public void mapConcurrently_findFirst_cancelledMapperPropagatesInterruption_winnerReturned() {
    ...
    Happenstance<String> happens = Happenstance.<String>builder()
        .sequence("2 running", "1 returns")
        .build();
    assertThat(
        Stream.of(1, 2, 3, 4)
            .gather(withMaxConcurrency(2).mapConcurrently(n -> {
              started.add(n);
              if (n == 1) { // the winner returns only after the loser is confirmed running
                happens.checkpoint("1 returns");
                return n;
              }
    ...
    assertThat(
        Stream.of(1, 2, 3, 4)
            .gather(withMaxConcurrency(3).mapConcurrently(n -> {
              started.add(n);
              if (n <= 2) { // both winners return only after the loser is confirmed running
                happens.checkpoint(n + " returns");
                return n;
              }
              happens.checkpoint(n + " running");
              try {
