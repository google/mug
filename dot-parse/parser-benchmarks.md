...
> The parser implementations for the comparison frameworks (including `antlr4`, `jparsec`, `fastparse`, `cats-parse`, `petitparser`, `parsecj`, `taker`, and `better-parse`) were generated by an AI agent and have **not** been officially reviewed or optimized by their respective framework authors or community experts. As such, they may not represent the most idiomatic or highly optimized implementations possible for those engines.
...
| Benchmark Scenario | `antlr4` | `dot-parse` | `jparsec` | `petitparser` | `fastparse` | `cats-parse` | `parsecj` | `taker` | `better-parse` | **Winner(s)** |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Complex JSON Payload** | $0.179$ | **$0.455$** 🚀 ☕ | $0.166$ | $0.110$ | $0.151$ | **$0.257$** 🚀 | $0.019$ | $0.080$ | $0.087$ | **`dot-parse`** 🚀 ☕<br>**`cats-parse`** 🚀 |

### Key Takeaways from the JSON Shootout

*   **Google's `dot-parse` Leads by a Wide Margin**:
    `dot-parse` is the fastest overall, achieving **$0.455$ ops/ms** and outperforming the next fastest framework, Scala's `cats-parse` ($0.257$ ops/ms), by **1.77x**.
*   **Combinator Library Comparison**:
    Within the Java-specific parser combinator libraries, `dot-parse` is **2.74x faster** than `jparsec` ($0.166$ ops/ms) and **23.9x faster** than `parsecj` ($0.019$ ops/ms).
*   **Scala Comparison**:
    `cats-parse` ($0.257$ ops/ms) outperforms the macro-based `fastparse` ($0.151$ ops/ms) by **1.70x** on this large, structured payload.
