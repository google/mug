...
*   **Vocabulary Scaling Boundary (500 Keywords)**: While macro-compiled PEG engines like `parboiled2` excel at small vocabulary sizes (120 rules), scaling to a 500-keyword alternation causes compile-time macro explosion, resulting in compiler stack overflow (`StackOverflowError` in `scalac` during typechecking). By contrast, runtime combinator libraries like `dot-parse`, `jparsec`, and `taker` scale cleanly without compile-time failure. Among these, `dot-parse` maintains high throughput because its in-memory Radix prefix trie scales lookup complexity by word length (O(k)) rather than vocabulary size (O(N)).
