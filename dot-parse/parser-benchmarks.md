...
| **Complex JSON with Comments** | $0.112$ | **$0.262$** ☕ | $0.107$ | $0.054$ | **$0.202$** 🚀 | $0.091$ | $0.002$ | $0.037$ | $0.036$ | **`dot`** ☕<br>**`fast`** 🚀 |
...
*   **Google's `dot-parse` Crowned Winner on JSON with Comments**:
    Under strict pure-combinator rules, `dot-parse` is the fastest third-party parser library on the JVM at **$0.262$ ops/ms** (running **$1.30\text{x}$ faster than idiomatic `fastparse`** and delivering **$64.1\%$ of Jackson's speed**).
*   **The Heap Allocation Tax in `fastparse`**:
    Reverting `fastparse` to its officially recommended, pure-combinator block comment skipper (`"/*" ~ (!"*/" ~ AnyChar).rep ~ "*/"`) resulted in a **$40.7\%$ performance drop** ($0.341 \to 0.202$ ops/ms). This represents the exact cost of the intermediate `Seq[Char]` heap allocations and primitive character boxing generated by the idiomatic combinator.
*   **`cats-parse` Allocation Pruning Success**:
    By pruning all intermediate `Tuple2` allocations (replacing `~` with `*>` and `<*`) and utilizing the pre-compiled native scanner `P.until(P.string("*/")).void`, `cats-parse` successfully bypassed intermediate lists, maintaining a strong **$0.091$ ops/ms** throughput (running at **$22.2\%$ of Jackson's speed**).
*   **`parsecj`'s Regex Tax on Delimiters**:
    No native, pre-compiled block comment skipper or `manyTill` combinator could be found in the library's repository or online tutorials. Consequently, developers must rely on the library's built-in `regex(...)` primitive to match block comments. While highly readable, evaluating a Java `Pattern` regex match at every single token boundary check is extremely heavy, causing a **$10\text{x}$ performance drop** ($0.020 \to 0.002$ ops/ms) due to continuous `Matcher` allocations on the hot path.
