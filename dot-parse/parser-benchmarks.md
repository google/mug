...
*   **Inability to Leverage Efficient Execution Modes**:
    ...
    *   **JavaCC (`htmlUnit`)**: JavaCC compiles token transitions into strict stateful classes that are not thread-safe. To parse concurrently or safely in a multi-threaded application, the runtime has to allocate a new parser instance for every single parse request, paying a heavy instantiation and GC tax. It also lacks vectorized scanning capabilities for comments and blocks, falling back to slow character-by-character DFA loops.
