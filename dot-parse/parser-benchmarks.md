...
*   **Vocabulary Scaling Boundary (500 Keywords)**: When scaling from a 12-keyword vocabulary to a **500-keyword vocabulary**, macro-compiled PEG engines like `parboiled2` hit a compile-time scaling limit, failing with compiler stack overflow (`StackOverflowError` in `scalac` during typechecking). Across all 11 remaining runtime combinator and parser libraries benchmarked under full warmup:
    *   **Case-Sensitive (500 CS)**: `dot-parse` leads overall at **167.45 ops/ms**, outperforming #2 `parboiled` (45.32 ops/ms) by 3.69x and #3 `antlr4` (28.55 ops/ms) by 5.86x. Because `dot-parse` compiles keyword alternations into an in-memory Radix prefix trie, lookup complexity scales by word length (O(k)) rather than vocabulary size (O(N)). By contrast, combinator libraries without prefix tries experience significant throughput reduction: `fastparse` drops by 47.6x (from 80 to 1.68 ops/ms), `cats-parse` drops by 29.5x (from 86 to 2.92 ops/ms), and `jparsec` drops by 29.2x (from 93 to 3.18 ops/ms).
    *   **Case-Insensitive (500 CI)**: `antlr4` leads overall (23.61 ops/ms), followed by `dot-parse` at **17.63 ops/ms** (#1 among combinator libraries). When generating 16 capitalization permutations per 4-character prefix across 500 keywords (~8,000 trie branches), `dot-parse` outperforms `jjparse` (3.55 ops/ms) by 4.96x, `cats-parse` (3.14 ops/ms) by 5.61x, `jparsec` (2.55 ops/ms) by 6.91x, and `fastparse` (1.58 ops/ms) by 11.16x.
