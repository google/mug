...
| **Nested Block Comment** | $1,106$ | **$8,791$** 🚀 ☕ | $1,907$ | $907$ | $4,143$ | $1,968$ | $770$ | $5,406$ | $254$ | $525$ | $581$ | $18$ | **`dot`** 🚀 ☕ |
 ...
 | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
 | **Simple Type (`String`)** | $1,554$ | **$6,926$** 🚀 ☕ | $1,410$ | $2,714$ | $7,093$ | $2,544$ | $486$ | **$13,846$** 🚀 | **`parboiled2`** 🚀<br>Java: **`dot`** 🚀 ☕ |
 | **Fully Qualified (`java.lang.String`)** | $802$ | **$2,768$** 🚀 ☕ | $547$ | $1,683$ | $4,468$ | $1,681$ | $281$ | **$5,920$** 🚀 | **`parboiled2`** 🚀<br>Java: **`dot`** 🚀 ☕ |
...
* **Analysis**:
  ...
  * **Monadic Baseline**:
    Classic `scala-pc` ($2.5\text{k}$) is **$6\text{x}$ to $7\text{x}$ slower** than modern Scala combinators. Because it constructs nested monadic structures and boxes intermediate results, it incurs call-stack and allocation overhead.

  * **ANTLR4 Two-Phase Allocation Overhead**:
    `antlr4` ($1.6\text{k}$) shows lower throughput here due to its compiler-grade two-phase parsing architecture (Lexer + Parser). On every micro-input execution, ANTLR4 must allocate a new `CharStream`, a new `Lexer`, a new `CommonTokenStream`, a new `Parser`, and individual `CommonToken` objects for every single token scanned, adding object allocation overhead.
...
* **Analysis**:
  ...
  * **`taker` String Scanning Performance**:
    On escaped strings, `taker` shows the highest throughput at **$9,580$ ops/ms**. It is optimized for string scanning with lower escape processing overhead.

  * **Bulk Scanning & Regex Delegation**:
    Libraries that support native bulk-scanning primitives or native Java Regex delegation perform well. `scala-pc` ($3.7\text{M}$ simple, $2.8\text{M}$ escaped) performs well here because its string rule compiles into a single Scala `Regex` (`stringVal.r`), delegating matching to Java's native regex engine.

  * **Character-by-Character PEG Cost**:
    On simple strings, `parboiled2` ($8.2\text{M}$) is slower than `cats-parse` ($18.1\text{M}$) and `fastparse` ($17.5\text{M}$) because it must rely on character-level PEG rules (`zeroOrMore(esc | normal)`). This introduces character branching, heap checking, and stack updates on every character matched, which is slower than bulk-scanning primitives.
...
* **Analysis**:
  ...
  * **`parboiled2` & `dot-parse` Performance Comparison**:
    `parboiled2` ($342$ ops/ms) and `dot-parse` ($249$ ops/ms) show the highest throughput overall through compile-time macro-optimized bytecode and zero-allocation runtime prefix-tries respectively.
  * **Trie Dispatch Implementation**:
    `dot-parse` (`anyOf`) compiles keyword alternatives into optimized **Radix Prefix Tries**, bypassing sequential backtracking. By compiling its branching nodes into a flat lookup table (array of size 256) when using `.precomputeForAscii()`, it achieves **$249$ ops/ms**.
  * **`parboiled2` Macro-Compiled Prefix Trie**:
    `parboiled2` achieves **$342$ ops/ms** by compiling the string alternatives directly into a character-matching prefix-trie branch structure in JVM bytecode.
...
* **Analysis**:
  ...
  * **`parboiled2` Case-Insensitive Macro Compilation**:
    `parboiled2` shows the highest throughput at **$227$ ops/ms** because its Scala macros compile case-insensitive string choices (`ignoreCase("select") | ...`) into trie-based character-matching branch bytecode.

  * **`dot-parse` Case-Insensitive Permutation Trie**:
    `dot-parse` shows the highest throughput among Java libraries at **$171$ ops/ms**. Its prefix-trie compiler precomputes capitalization permutations of the first 4 characters at startup, maintaining an optimized $O(1)$ dispatch.

  * **Backtracking Penalties**:
    Libraries that do not precompute prefix-tries must backtrack through all 120 options sequentially, resulting in significantly lower throughput. For example, `parsecj` drops to **$24$ ops/ms** because it backtracks sequentially through 120 individual compiled case-insensitive regex patterns.
...
* **Analysis**:
  ...
  * **`parboiled2` Compile-Time Macro Performance**:
    `parboiled2` shows the highest throughput in the recursive expression benchmark, matching **1.61 million operations per second**, which is **$91\%$ faster** than `fastparse` ($0.84\text{M}$ ops/sec). By using compile-time macros, `parboiled2` compiles recursive PEG rules into inline bytecode methods that execute expression parsing as primitive loops.

  * **The `fastparse` Mutable State-Passing Pattern**:
    `fastparse` ($845$ ops/ms) performs well by using compile-time macro expansion to rewrite declarative combinators into a **single mutable context object passing pattern (`ParsingRun`)**.

  * **Traditional Combinator Allocation Penalty**:
    In contrast, libraries like `dot-parse` ($526$ ops/ms) and `cats-parse` ($373$ ops/ms) must allocate a fresh, temporary `MatchResult` wrapper on every addition, multiplication, and parenthesis nesting. `scala-pc` ($170\text{ ops/ms}$) is **$9.5\text{x}$ slower** than `parboiled2` due to this stack and boxing overhead.
...
* **Analysis**:
  ...
  * **`dot-parse` Native Flat Character Scan**:
    `dot-parse` achieves **8.7 million operations per second** by utilizing its native `nestedBy("/*", "*/")` primitive. Rather than constructing a recursive tree of parser combinator objects, `nestedBy` scans the character stream in a single flat loop, tracking nesting depth in a primitive integer counter. This minimizes CPU and memory overhead, outperforming even `parboiled2` ($5.4\text{M}$) and `fastparse` ($4.1\text{M}$).

  * **`parboiled2` Recursive PEG Compilation**:
    `parboiled2` ($5.4\text{M}$ ops/ms) ranks second. Because its macro-based compiler compiles the recursive rule `comment` into a set of inline, direct branch-based bytecode methods, it bypasses call-stack allocations and executes lookahead negation (`!"*/" ~ ANY`) at near-native speeds.
