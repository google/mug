...
public final class JavaTypeParser {
  ...
  private static final Parser<String> PRIMITIVE_TYPE = anyOf(
      string("int"), string("double"), string("float"), string("long"), string("short"),
      string("byte"), string("char"), string("boolean"), string("void"));

  private static final Parser<String> TYPE_NAME =
      anyOf(PRIMITIVE_TYPE, sequence(one("[A-Z]"), zeroOrMore("[a-zA-Z0-9_]")).source());
  ...
  private static final Parser<String> UNICODE_ESCAPE =
      string("u").then(BMP_CODE_UNIT).map(Object::toString);
  ...
  private static final Parser<?> INTEGER_PART = sequence(string("-").optional(), digits());
  ...
  public static final Parser<JavaType> JAVA_TYPE = Parser.define(
      selfType -> {

        // Annotation parser (mutually recursive with selfType)
        Parser<JavaAnnotation> annotation = Parser.define(
            selfAnno -> {
              ...
              Parser<AnnotationValue> annotationValue = Parser.define(
                  selfVal -> {
                    Parser<AnnotationValue> stringVal =
                        STRING_LITERAL.map(AnnotationValue.StringValue::new);
                    Parser<AnnotationValue> numberVal =
                        NUMBER_VAL.map(AnnotationValue.NumberValue::new);
                    Parser<AnnotationValue> classLiteralVal =
                        selfType.followedBy(".class").map(AnnotationValue.ClassLiteralValue::new);
                    Parser<AnnotationValue> nestedAnnoVal =
                        selfAnno.map(AnnotationValue.AnnotationValueHolder::new);
              ...
              Parser<Map<String, AnnotationValue>> namedParams = zeroOrMoreDelimited(
                      word().followedBy("="), annotationValue, ",", BiCollectors.toMap())
                  .between("(", ")");
              ...
              return string("@").then(
                      sequence(annotationName, params.orElse(Map.of()), JavaAnnotation::new));
            });

        // Type segment (e.g. @NonNull Outer<String>)
        // We combine the optional prefix (annotations) and mandatory type name using
        // sequence(OrEmpty, Parser, BiFunction)
        Parser<Map.Entry<List<JavaAnnotation>, String>> annosAndName =
            sequence(annotation.zeroOrMore(), TYPE_NAME, AbstractMap.SimpleImmutableEntry::new);
