...
public final class RobertFischerJsonParserTest extends AbstractJsonParserTest {

  @Override
  protected JsonValue parse(String input) throws Exception {
    RobertFischerJsonParser parser = new RobertFischerJsonParser(input);
  ...
  private static JsonValue convert(Object parsed) {
    if (parsed == null) {
      ...
      return new JsonArray(list.stream().map(RobertFischerJsonParserTest::convert).toList());
  ...
  public void parseInvalidNumbers_throwsException() {
    // Throws TokenMgrError (an Error, not Exception) for some invalid tokens
  ...
  public void parseStringWithInvalidEscapes_throwsException() {
    // Throws TokenMgrError for invalid escapes
  ...
  public void parseInvalidJson_throwsException() {
    // It supports unquoted keys, so {a: 1} is valid. But other structures are invalid.
  ...
  public void parseWithBlockComment() throws Exception {
    // Greedy block comment bug: we can only have a single comment at the end or no other comments,
    // otherwise they swallow everything between them.
  ...
  @Test
  public void parseWithMultipleBlockComments() throws Exception {
    JsonValue result = parse("""
        {
          /* comment 1 */
          "a": 1,
        ...
        """);
    assertThat(result).isEqualTo(new JsonObject(java.util.Map.of("a", new JsonNumber(1.0), "b", new JsonNumber(2.0))));
  }
  ...
  @Test
  public void parseIntegerExponent() throws Exception {
    assertThat(parse("-5e-122")).isEqualTo(new JsonNumber(-5e-122));
    assertThat(parse("3e10")).isEqualTo(new JsonNumber(3e10));
  }
}
