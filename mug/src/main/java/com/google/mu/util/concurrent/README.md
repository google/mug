...
It makes sense to do these IO-bound operations concurrently. JDK's `StructuredTaskScope` ([JEP 453](https://openjdk.org/jeps/453)) remains in preview across JDK 21+, and its API can still feel verbose for simple fanout operations. So we created a simpler API ([Fanout](https://google.github.io/mug/apidocs/com/google/mu/util/concurrent/Fanout.html)). It focuses on the most common fanout use case where the `ShutdownOnFailure` strategy is sufficient.
