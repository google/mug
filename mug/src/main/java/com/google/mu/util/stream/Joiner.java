 ...
 *
 * <p>When used to join a pair of objects (for example in a BiStream), it can be used for
 * BiFunction:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Alternatively, it can be used as a Collector. For example, {@code
 * names.collect(Joiner.on(','))} is equivalent to {@code names.collect(Collectors.joining(","))}.
 *
 * <p>Except that JDK {@code joining()} requires the inputs to be strings; while Joiner can join any
 * input, e.g. numbers.
 *
 ...
 *
 * <p>You can also chain {@link #between} to further enclose the joined result between a pair of
 * strings. The following code joins a list of ids and their corresponding names in the format of
 * {@code "[id1=name1, id2=name2, id3=name3, ...]"} by first joining each pair with {@code '='} and
 * then joining the {@code "id=name"} entries with {@code ','}, finally enclosing them between
 * {@code '['} and {@code ']'}:
 *
 * <pre>{@code
 * BiStream.zip(userIds, names)
 ...
 * collect(Joiner.on(...).skipNulls())} or {@code collect(Joiner.on(...).skipEmpties())}
 * respectively.
 *
...
public final class Joiner implements Collector<Object, Joiner.FasterStringJoiner, String> {
  ...
  public String join(Collection<?> collection) {
    if (collection.size() == 1 && prefix.isEmpty() && suffix.isEmpty()
   ...
   *
   * <p>For example both {@code Joiner.on(',').between('[', ']').join(List.of(1, 2))} and {@code
   * Joiner.on(',').between('[', ']').join(1, 2)} return {@code "[1,2]"}.
   */
   ...
   * Returns an instance that wraps the join result between {@code before} and {@code after}.
   *
   * <p>For example both {@code Joiner.on(',').between("[", "]").join([1, 2])} and {@code
   * Joiner.on(',').between("[", "]").join(1, 2)} result in {@code "[1,2]"}.
   */
  ...
  /**
   * Returns a Collector that skips null and empty string inputs and joins the remaining using this
   * Joiner.
   */
